...
export class UserRepository {
  ...
  async searchUsers(
    ...
    const [count, users] = await Promise.all([
      this.dbService.user.count({ where: filter }),
  ...
  async getAllUsersWhere(filter: Prisma.UserWhereInput, pagination: PaginationDto) {
    ...
    const [count, users] = await Promise.all([this.dbService.user.count({ where: filter }), this.dbService.user.findMany({ where: filter, ...(getPaginationQuery(pagination)) })])
