...
export class ThemeService {
  ...
  async updateTheme(uid: string, updateThemeDto: UpdateThemeDto) {
    ...
    if (
      updateThemeDto.startDate &&
      !updateThemeDto.endDate &&
      updateThemeDto.startDate > theme.endDate
    ) {
      throw new Error('invalidDates');
    } else if (
      updateThemeDto.endDate &&
      !updateThemeDto.startDate &&
      updateThemeDto.endDate < theme.startDate
    ) {
      throw new Error('invalidDates');
    } else if (
      updateThemeDto.startDate &&
      updateThemeDto.endDate &&
      updateThemeDto.startDate > updateThemeDto.endDate
    ) {
      throw new Error('invalidDates');
    }
...
export class GroupService {
  ...
  async createGroup(createGroupDto: CreateGroupDto) {
    ...
    const theme = await this.themeRepository.findThemeByUid(
      createGroupDto.themeId
    );
    if (!theme) {
      throw new Error('themeDoesntExists');
    }
    ...
    if (
      createGroupDto.startDate < theme.startDate ||
      createGroupDto.endDate > theme.endDate
    ) {
      throw new Error('invalidDates');
    }
  ...
  async updateGroup(uid: string, updateGroupDto: UpdateGroupDto) {
    ...
    if (group.startDate !== updatedGroup.startDate) {
      ...
      const assignings: AssigningGroup[] =
        await this.assigningGroupRepository.getByGroupUid(uid);
      for (const assigning of assignings) {
        const resiver = await this.getResiver(
          assigning.enrolledType,
          assigning.enrolledUid
        );
        if (resiver) {
          await this.taskService.rescheduleFormationReminder(
            assigning.enrolledType,
            updatedGroup,
            resiver
          );
  ...
  async deleteGroup(uid: string) {
    ...
    const assigningGroups =
      await this.assigningGroupRepository.getByGroupUid(uid);
    for (const assigningGroup of assigningGroups) {
      // cancel the formation reminder
      this.taskService.cancelFormationReminder(
        assigningGroup.enrolledType,
        assigningGroup.enrolledUid,
        assigningGroup.groupUid
      );
      // delete the assigning group
      await this.assigningGroupRepository.deleteAssigningGroup(
        assigningGroup.uid
      );
    }
...
export class AnimatorService {
  ...
  async createAnimator(createAnimatorDto: CreateAnimatorDto) {
    ...
    const organization = await this.organizationRepository.findByCnss(
      createAnimatorDto.organizationId
    );
    if (!organization) {
      throw new Error('organizationDoesntExists');
    }
    ...
    const animator = await this.animatorRepository.getByEmail(
      createAnimatorDto.email
    );
  ...
  async getAnimator(uid: string): Promise<Animator> {
    if (!uid) throw new Error('uidRequired');
    const animator: Animator | null =
      await this.animatorRepository.getByUid(uid);
    if (!animator)
  ...
  async updateAnimator(updateAnimatorDto: UpdateAnimatorDto, uid: string) {
    ...
    if (updateAnimatorDto.email && updateAnimatorDto.email !== animator.email) {
      const animatorByEmail = await this.animatorRepository.getByEmail(
        updateAnimatorDto.email
      );
      if (animatorByEmail) {
...
export class AssigningGroupService {
   ...
   */
  async checkOverlappingDates(
    assignToGroupDto: AssignToGroupDto
  ): Promise<boolean> {
    const enrolledGroups = await this.getByEnrolleds(
      assignToGroupDto.enrolledUid,
      assignToGroupDto.enrolledType
    );
    if (enrolledGroups.length === 0) return false;

    const group = await this.groupRepository.getByUid(
      assignToGroupDto.groupUid
    );
    ...
    for (const enrolledGroup of enrolledGroups) {
      ...
      if (
        enrolledGroup.startDate < group.endDate &&
        enrolledGroup.endDate > group.startDate
      ) {
        return true;
      }
    }
  ...
  async createAssigningGroup(assignToGroupDto: AssignToGroupDto) {
    ...
    switch (assignToGroupDto.enrolledType) {
      case EnrolledType.Animator:
        const animator = await this.animatorRepository.getByUid(
          assignToGroupDto.enrolledUid
        );
        if (!animator) {
          throw new Error('animatorDoesntExists');
        ...
        break;
      case EnrolledType.Formator:
        const formator = await this.formatorRepository.getByUid(
          assignToGroupDto.enrolledUid
        );
        if (!formator) {
        ...
        break;
      case EnrolledType.Participant:
        const participant = await this.participantRepository.getByUid(
          assignToGroupDto.enrolledUid
        );
        if (!participant) {
    ...
    const group: Group | null = await this.groupRepository.getByUid(
      assignToGroupDto.groupUid
    );
    if (!group) {
      throw new Error('groupDoesntExists');
    ...
    const assigningGroup =
      await this.assigningGroupRepository.createAssigningGroup(
        assignToGroupDto
      );

    await this.taskService.scheduleFormationReminder(
      assignToGroupDto.enrolledType,
      group,
      resiver
    );
  ...
  async getByEnrolleds(enrolledUid: string, enrolledType: EnrolledType) {
    ...
    switch (enrolledType) {
      ...
      case EnrolledType.Participant:
        const participant =
          await this.participantRepository.getByUid(enrolledUid);
        if (!participant) {
          throw new Error('participantDoesntExists');
        }
        break;
    }
    return await this.assigningGroupRepository.getByEnrolleds(
      enrolledUid,
      enrolledType
    );
  }
  ...
  async getByGroupUid(groupUid: string, enrolledType: EnrolledType) {
    ...
    return await this.assigningGroupRepository.getByGroupUidAndEnrolledType(
      groupUid,
      enrolledType
    );
  }
...
export class FormatorService {
  ...
  async createFormator(createFormatorDto: CreateFormatorDto) {
    // check if the organization exists
    const organization = await this.organizationRepository.findByCnss(
      createFormatorDto.organizationId
    );
    if (!organization) {
      throw new Error('organizationDoesntExists');
    }
    ...
    const formator = await this.formatorRepository.getByEmail(
      createFormatorDto.email
    );
  ...
  async getByEmail(email: string): Promise<Formator> {
    ...
    const formator: Formator | null =
      await this.formatorRepository.getByEmail(email);
    if (!formator)
  ...
  async getByUid(uid: string): Promise<Formator> {
    if (!uid) throw new Error('uidRequired');
    const formator: Formator | null =
      await this.formatorRepository.getByUid(uid);
    if (!formator)
  ...
  async updateFormator(updateFormatorDto: UpdateFormatorDto, uid: string) {
    ...
    if (updateFormatorDto.email && updateFormatorDto.email !== formator.email) {
      const formatorByEmail = await this.formatorRepository.getByEmail(
        updateFormatorDto.email
      );
      if (formatorByEmail) {
        throw new Error('emailAlreadyExists');
      }
...
export class ParticipantService {
  ...
  async createParticipant(createParticipantDto: CreateParticipantDto) {
    const participantByEmail = await this.participantRepository.getByEmail(
    ...
    const participantByCnss = await this.participantRepository.getByCnss(
      createParticipantDto.cnss
    );
    ...
    const newParticipant =
      await this.participantRepository.createParticipant(createParticipantDto);
  ...
  async getParticipant(uid: string): Promise<Participant> {
    ...
    const participant: Participant | null =
      await this.participantRepository.getByUid(uid);
    if (!participant)
  ...
  async updateParticipant(
    ...
    return await this.participantRepository.updateParticipant(
