...
export class OrganizationRepository {
  ...
  async updateOrganization(
    ...
    const params: UpdateItemCommandInput = {
      ...
      UpdateExpression:
        'SET #name = :name, #freeTrial = :freeTrial, #updatedAt = :updatedAt',
      ...
      ExpressionAttributeValues: {
        ...
        ':freeTrial': {
          N:
            updateOrganizationDto.freeTrial?.toString() ??
            organization.freeTrial.toString(),
        },
    ...
    for (const [key, value] of Object.entries(fieldsToUpdate)) {
      if (value !== undefined) {
        ...
        expressionAttributeValues[attributeValue] =
          this.dbService.convertToAttributeValue(value);
      }
    }
...
export class AnimatorRepository {
  ...
  async updateAnimator(
    updateAnimatorDto: UpdateAnimatorDto,
    uid: string
  ): Promise<Animator> {
  ...
  async getAllAnimators(page: number, limit: number, name?: string) {
    ...
    return {
      items: paginatedItems.map((item: Record<string, any>) =>
        this.dbService.mapDynamoDBItemToObject(item)
      ),
      total: Items.length,
      page,
...
export class AssigningGroupRepository {
  ...
  constructor(
    ...
    this.assigningGroupPrimaryKey =
      this.dbConstants.getPrimaryKey('AssigningGroups');
  }
  ...
  ): Promise<AssigningGroup[]> {
    const params: QueryCommandInput = {
      TableName: this.assigningGroupTableName,
      IndexName: 'EnrolledIndex',
      KeyConditionExpression:
        'enrolledUid = :enrolledUid AND enrolledType = :enrolledType',
      ExpressionAttributeValues: {
   ...
   * @returns The assigning group found or null if not found.
   */
  async getByGroupUidAndEnrolledType(
    groupUid: string,
    enrolledType: EnrolledType
  ): Promise<AssigningGroup[]> {
    const params: QueryCommandInput = {
      TableName: this.assigningGroupTableName,
      KeyConditionExpression:
        'groupUid = :groupUid AND enrolledType = :enrolledType',
      ExpressionAttributeValues: {
        ':groupUid': { S: groupUid },
        ':enrolledType': { S: enrolledType },
      },
...
export class FormatorRepository {
  ...
  async updateFormator(
    updateFormatorDto: UpdateFormatorDto,
    uid: string
  ): Promise<Formator> {
