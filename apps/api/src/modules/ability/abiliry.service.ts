...
export class AbilityService {
  ...
  async getAbility(identifier: string): Promise<AbilityInterface | null> {
    ...
    try {
      ...
      return this.dbService.mapDynamoDBItemToObject(
        Items[0]
      ) as AbilityInterface;
    } catch (_) {
  ...
  async deassignAbility(
    ability: AbilityInterface & { PK: string; SK: string }
  ) {
    // Create the params
  ...
  async deassignAbilityByUid(deleteAbilityDto: DeassignAbilityDto) {
    const ability = await this.getAbility(
      `${deleteAbilityDto.uid}#${deleteAbilityDto.abilityType}`
    );
    if (!ability) {
  ...
  async eraseAbilities(userUid: string) {
    ...
    for (const ability of abilities) {
      await this.deassignAbility(
        this.dbService.mapDynamoDBItemToObject(ability)
      );
    }
  }
