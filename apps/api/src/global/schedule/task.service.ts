...
import {
  EnrolledType,
  Group,
} from 'src/modules/organization/model/group.model';
...
export class TaskService {
  ...
  async scheduleFormationReminder(
    enrolledType: EnrolledType,
    group: Group,
    resiver: Animator | Formator | Participant
  ) {
    try {
      const oneWeekBefore = new Date(
        Number(group.startDate) - 7 * 24 * 60 * 60 * 1000
      );
      const oneDayBefore = new Date(
        Number(group.startDate) - 24 * 60 * 60 * 1000
      );
      ...
      if (Number(oneWeekBefore) > Date.now()) {
        ...
        template.html = template.html.replace(
          '{{name}}',
          resiver.firstName + ' ' + resiver.lastName
        );
        template.html = template.html.replace('{{formationName}}', group.theme);
        template.html = template.html.replace(
          '{{formationStartDate}}',
          new Date(Number(group.startDate)).toISOString()
        );
  ...
  cancelFormationReminder(
    ...
    this.cancelScheduledEmail(
      `${enrolledType}-${resiverUid}-${groupUid}-one-week`
    );
    this.cancelScheduledEmail(
      `${enrolledType}-${resiverUid}-${groupUid}-one-day`
    );
    this.cancelScheduledEmail(
      `${enrolledType}-${resiverUid}-${groupUid}-started`
    );
    console.log(
      `Formation reminder cancelled for ${enrolledType}-${resiverUid}-${groupUid}`
    );
  }

  async rescheduleFormationReminder(
    enrolledType: EnrolledType,
    group: Group,
    resiver: Animator | Formator | Participant
  ) {
