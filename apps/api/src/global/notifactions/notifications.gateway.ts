...
export class NotificationsGateway
  ...
  async handleSendNotification(client: Socket, payload: any) {
    ...
    try {
      ...
      const notification: Notifications =
        await this.notificationService.createNotification(uid, message);
  ...
  async handleViewNotification(client: Socket, payload: any) {
    ...
    try {
      ...
      userSockets?.forEach((socket) =>
        socket.emit('notificationViewed', { notification })
      );
    } catch (error) {
      client.emit('error', {
  ...
  async handleBroadcastNotification(client: Socket, payload: any) {
    ...
    try {
      ...
      const notifications: Notifications[] =
        await this.notificationService.broadcastToUsers(uids, message);
